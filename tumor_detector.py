...
def main(args):
    ...
    for i in folds:
        ...
        df = pd.read_csv('{}/splits_{}.csv'.format(args.split_dir, i))
        train_list = df['train'].dropna().tolist()
        val_list = df['val'].dropna().tolist()
        test_list = df['test'].dropna().tolist()
        train_dataset = WSI_Patches_Image(train_list, args.patch_image_path, training=True, maximum=500)
        val_dataset = WSI_Patches_Image(val_list, args.patch_image_path, training=False, maximum=200)
        test_dataset = WSI_Patches_Image(test_list, args.patch_image_path, training=False, maximum=200)
...
parser.add_argument('--patch_image_path', type=str, help='dataset wsi path')
...
parser.add_argument('--task', default='tumor_classifier', type=str, choices=['tumor_classifier', 'tumor_classifier_luad'])
