...
class WholeSlideImage(object):
    def __init__(self, path):
        ...
        self.name = os.path.basename(path).replace('.svs', '').replace('.tif', '')
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, only_tumor=False,
    ...
    def _tumor_filter_save_hdf5(self, asset_dict, patch_level, patch_size, normalize, save_path_hdf5, attr_dict=None,
                                mode='w', batch_size=200, ratio=0.95):
        per_batch_img, per_batch_xy, coordinates = [], [], []
        asset_list = asset_dict['coords'].tolist()
        index = 0
        self.tumor_detect_model.eval()
        self.extract_feature_model.eval()
        with torch.no_grad():
            for idx, coord in tqdm(enumerate(asset_list)):
                img = self.wsi.read_region(coord, patch_level, (patch_size, patch_size)).convert('RGB').resize(
                    (256, 256), resample=0)
                if normalize:
                    try:
                ...
                if index >= batch_size or idx == len(asset_list) - 1:
                    ...
                    logits = self.tumor_detect_model(per_batch).to(torch.device('cpu'))
                    Y_prob = F.softmax(logits, dim=1)
                    labels = Y_prob[:, 1].ge(ratio).numpy()
                    if any(labels):
                        features = self.extract_feature_model(per_batch[labels])
                        features = features.cpu().numpy()
                        coordinates = np.array(per_batch_xy)[labels]
                        if mode == 'w':
                            save_hdf5(save_path_hdf5, {'coords': np.array(coordinates), 'features': features},
                                      attr_dict, mode='w')
                            mode = 'a'
                        elif mode == 'a':
                            save_hdf5(save_path_hdf5, {'coords': np.array(coordinates), 'features': features},
...
if __name__ == '__main__':
    a = WholeSlideImage('E:/wsi-test2/10-tsjy2034203-1.tif')
    print(a.wsi.get_best_level_for_downsample(65.0))
